...
describe("MusicNFTMarketplace", function (){
    ...
    let URI = "https://ipfs.filebase.io/ipfs/bafybeibz7qz53yekoslgmmnyxd6a2sozw6chxozfrruukainbtmvjbgk34/"
    ...
    describe("Deployment", function(){
        ...
        it("Should track name, symbol, URI, royalt fee and artist", async function() {
            const nftName = "SPotiFi"
            const nftSymbol = "SPOTI"
        ...
        it("Should mint then list all the music nfts", async function() {
            expect(await nftMarketplace.balanceOf(nftMarketplace.address)).to.equal(4);
    ...
    describe("Buying tokens", function(){
        it("Should update the seller to zero address, transfer NFT, pay seller, pay royalty to artist and emit a MarketItemBought event", async function(){
            ...
            expect(+fromWei(deployerFinalEthBal)).to.equal(+fromWei(deployerInitialEthBal) + +fromWei(prices[0]))
            //Artist should revieve royalty.
            expect(+fromWei(artistFinalEthBal)).to.equal(+fromWei(artistIntialEthBal) + +fromWei(royaltyFee))
    ...
    describe("Reselling tokens", function(){
        ...
        it("Should track resale item, incr. ether bal by royalty fee, transfer NFT to marketplace and emit MarketItemRelisted event", async function(){
            ...
            //Except final market bal to equal initial + royalty fee
    ...
    describe("Getter functions", function(){
        ...
        beforeEach(async function(){
            ...
            await (await nftMarketplace.connect(user2).buyToken(3, { value: prices[3] })).wait();
        })
